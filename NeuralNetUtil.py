...
def getNNRiceData(fileString = "datasets/Rice_Cammeo_Osmancik.xlsx"):
    df = pd.read_excel(fileString)
    # Normalize the dataframe by column
    columns_to_normalize = ['Area', 'Perimeter', 'Major_Axis_Length', 'Minor_Axis_Length', 'Eccentricity', 'Convex_Area', 'Extent']
    for column in df:
    ...
    for i in range(df.shape[0]):
        inVec = [0, 0, 0, 0, 0, 0, 0]
        outVec = [0, 0]  # Cammeo or Osmancik (Rice Category) Output
...
def buildExamplesFromExtraData(size = 500):
    rice_examples = getNNRiceData()
    random.shuffle(rice_examples)
    riceTrainList = rice_examples[:-size]
    riceTestList = rice_examples[-size:]
    return riceTrainList, riceTestList
