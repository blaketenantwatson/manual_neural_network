...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        totalModification = 0
        """YOUR CODE"""
        biased_inputs = [1.0] + inActs
        for i in range(len(biased_inputs)):
            update = alpha * delta * biased_inputs[i]
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                newActs.append(perceptron.sigmoidActivation(curr))
            curr = newActs
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:  # For each example
            ...
            outDelta = []  # Empty output layer delta list
            # Iterate through all output layer neurons
            for outputNum in range(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(output[-2])
                error = example[1][outputNum] - output[-1][outputNum]
                delta = gPrime * error
            ...
            for layerNum in range(self.numHiddenLayers - 1, -1, -1):
                ...
                for neuronNum in range(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(output[layerNum])
                    weight = 0.0
                    for i, perceptron in enumerate(nextLayer):
                        weight += perceptron.weights[neuronNum + 1] * deltas[0][i]
                    delta = gPrime * weight
            ...
            """
            for numLayer, layer in enumerate(self.layers):
                for numNeuron in range(len(layer)):
                    weightMod = layer[numNeuron].updateWeights(output[numLayer], alpha, deltas[numLayer][numNeuron])
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    iteration = 0
    trainError = 0
    weightMod = 9999  # This is set high so that at least one iteration of the while loop is run.
    ...
    pbar = tqdm(total=maxItr)

    while iteration < maxItr and weightChangeThreshold < weightMod:
    ...
    for example in examplesTest:
        rounded_output = [round(item) for item in nnet.feedForward(example[0])[-1]]  # Round all output values
        if rounded_output == example[1]:
            testCorrect += 1
        else:
            testError += 1

    testAccuracy = testCorrect / len(examplesTest)  # num correct/num total # NOTE: Might need to float the length
